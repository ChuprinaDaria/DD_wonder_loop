...
class OpenAIService:
    ...
    def _check_prohibited_words(self, data: dict) -> tuple[bool, str]:
        ...
        forbidden_keywords = [
            ...
            # Матюки та лайка (укр)
            "бл*ть", "бл*дь", "х*й", "п*зда", "сука", "лайно", "гівно", "дермо",
            "йобати", "їбати", "піздець", "хуйня", "ідіот", "дебіл", "мудак",
            "курва", "шлюха", "повія", "дурак", "тупий", "ублюдок", "падла",
            
            # Матюки та лайка (англ)
            "fuck", "shit", "bitch", "damn", "hell", "asshole", "bastard",
            "whore", "slut", "cunt", "dick", "cock", "pussy", "motherfucker",
            "bullshit", "dumbass", "retard", "idiot", "stupid", "moron",
            
            # Політика (укр)
            "зеленський", "путін", "байден", "трамп", "війна", "армія", "фронт",
            "військовий", "солдат", "зсу", "всу", "вдв", "окупант", "рашист",
            "хохол", "москаль", "кацап", "бандера", "сталін", "гітлер",
            "політик", "президент", "прем'єр", "депутат", "рада", "парламент",
            
            # Політика (англ)  
            "zelensky", "putin", "biden", "trump", "war", "army", "military",
            "soldier", "politics", "president", "minister", "parliament",
            "government", "election", "vote", "campaign", "nazi", "fascist"
...
• Для якого типу шкіри засіб: {стиль типу шкіри}
{рядок з ціною або обміном - ВСТАВИТИ БЕЗ ЗМІН}
• Локація: {місто}
• Доставка: {доставка з гумором}
...
ТИПИ ШКІРИ (для поля "• Для якого типу шкіри засіб:"):
...
Для якого типу шкіри засіб:: {data.get('skin_type', '')}
Місто: {data.get('city', '')}
...
Поверни ТІЛЬКИ адаптований текст у вказаній структурі, БЕЗ пояснень."""
    ...
    def _generate_fallback_post(self, data: dict) -> str:
        ...
        lines = [
            ...
            f"• Для якого типу шкіри засіб:: {data.get('skin_type', 'різна')}"
        ]
